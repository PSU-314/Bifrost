...
mkdir -p login-server/pki/server login-server/pki/root-ca/
...
cp pki_staging/server/server.crt pki_staging/server/server-chain.pem
...
cp pki_staging/client/bifrost.crt pki_staging/client/bifrost-chain.pem

cp pki_staging/root-ca/root-ca.crt login-server/pki/root-ca/root-ca.crt
cp pki_staging/root-ca/root-ca.crt bifrost/certs/root-ca.crt

cp pki_staging/server/server-chain.pem login-server/pki/server/server-chain.pem
cp pki_staging/client/bifrost-chain.pem bifrost/certs/bifrost-chain.pem

cp pki_staging/server/server.key login-server/pki/server/server.key
