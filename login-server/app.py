...
class User(db.Model):
    ...
    shared_secret_hex = db.Column(
        db.String(64), nullable=True
    )  # Populated after 1 successful exchange
...
def _purge_expired_tokens() -> None:
    ...
    now = time.time()
    expired_tokens = ExchangeToken.query.filter(
    ...
    if username and password:
        if len(username) < 3 or len(password) < 6:
            return render_template(
                "page2.html", error="Username must be ≥ 3 chars and password ≥ 6 chars."
            )
...
def exchange_endpoint(pin: str):
    ...
    print("Received client public key: ", client_pub_hex)
    ...
    try:
        ...
        print("Shared Secret: ", secret.hex())
    ...
    print("Server public key: ", server_pub_hex)
